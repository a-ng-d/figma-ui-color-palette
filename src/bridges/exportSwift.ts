    ...
    JSON.parse(palette.getPluginData('data')).forEach(
      (color: PaletteDataItem) => {
        ...
        color.shades.forEach((shade) => {
          UIColors.unshift(
            `static let ${color.name.toLowerCase().replace(/[^a-zA-Z0-9]+(.)/g, (m, chr) => chr.toUpperCase()).replace(/[@/$^%#&!?,;:+=<>(){}\[\]"«»]/g, '')}${shade.name === 'source' ? 'Source' : shade.name} = Color(red: ${shade.gl[0].toFixed(3)}, green: ${shade.gl[1].toFixed(3)}, blue: ${shade.gl[2].toFixed(3)})`
          )
        })
        UIColors.unshift('')
