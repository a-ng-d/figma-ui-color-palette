...
const updateLocalVariables = (palette, i: number, j: number) => {
  ...
  if (palette.children.length == 1) {
    ...
    const collection: VariableCollection | undefined =
      figma.variables.getLocalVariableCollections()
        .find(collection => collection.id === paletteData.collectionId)

    if (collection != undefined) {
      const localVariables: Array<Variable> =
        figma.variables.getLocalVariables()
          .filter(localVariable => localVariable.variableCollectionId === collection.id)

      if (collection.name != name)
        collection.name = name

      const workingThemes = paletteData.themes
      ...
      workingThemes
        .forEach(theme => {
          const modeMatch = collection.modes.find(mode => mode.modeId === theme.modeId)
          ...
          theme.colors.forEach(color => {
            color.shades.forEach(shade => {
              const variableMatch = localVariables.find(localVariable => localVariable.id === shade.variableId)
      ...
      if (i > 1 || j > 1)
        ...
        figma.notify(`${notifications.join(' and ')} has been updated`)
