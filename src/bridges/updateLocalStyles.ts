  ...
  if (palette.children.length == 1) {
    ...
    JSON.parse(palette.getPluginData('data')).forEach((color: PaletteDataItem) => {
      color.shades.forEach(shade => {
        target = localStyles.filter(localStyle => localStyle.name === `${color.name}/${shade.name}`)[0]

        if (shade.hex != chroma([target.paints[0]['color'].r * 255, target.paints[0]['color'].g * 255, target.paints[0]['color'].b * 255]).hex()) {
          target.paints = [
            {
              type: 'SOLID',
              color: {
